...
```mermaid
flowchart TD
    A[Use Case] --> B{Error Cost?}
    B -->|Life-threatening| C{Reversible?}
    B -->|Financial / operational| D{Volume?}
    C -->|No| E[L1-L2: Human checkpoint required]
    C -->|Yes| F{Regulatory?}
    D -->|High volume| G{Domain stable?}
    D -->|Low volume| F
    F -->|Heavy compliance| E
    F -->|Light / none| H{Task complexity?}
    G -->|Stable| I[L3: Goal-driven viable]
    G -->|Unstable| F
    H -->|Multi-domain| J[L4: Multi-agent if speed-critical]
    H -->|Single domain| I
```
