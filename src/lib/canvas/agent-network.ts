...
const CYAN: Color = { r: 76, g: 201, b: 208 };
const TEAL: Color = { r: 59, g: 191, b: 166 };
const EMERALD: Color = { r: 66, g: 198, b: 139 };
...
export function initAgentNetwork(canvas: HTMLCanvasElement): () => void {
  ...
  function spawnHeroSignal() {
    ...
    let a = Math.floor(Math.random() * heroNodes.length);
    let b: number;
    do {
      b = Math.floor(Math.random() * heroNodes.length);
    } while (b === a);
    ...
    heroSignals.push({
      ...
      color: lerpColor(from.type.color, to.type.color, 0.5),
      trail: [],
    });
  }
  ...
  function animateHero(time: number) {
    ...
    for (let i = 0; i < heroNodes.length; i++) {
      for (let j = i + 1; j < heroNodes.length; j++) {
        ...
        if (d > heroW * 0.6) continue;
        const alpha = 0.03 * (1 - d / (heroW * 0.6));
        const mx = (a.x + b.x) / 2;
        const my = (a.y + b.y) / 2;
        const dx = b.x - a.x;
        const dy = b.y - a.y;
    ...
    for (let i = heroSignals.length - 1; i >= 0; i--) {
      ...
      s.p += s.speed;
      if (s.p >= 1) {
        heroNodes[s.to].targetBrightness = 1;
        heroSignals.splice(i, 1);
        continue;
      }
      ...
      if (s.trail.length > 2) {
        ...
        ctx.strokeStyle = rgba(s.color, 0.12);
        ctx.lineWidth = 2;
        ctx.stroke();
      }
      for (let t = s.trail.length - 1; t >= 0; t--) {
        ...
        ctx.beginPath();
        ctx.arc(tp.x, tp.y, (1 - tp.age) * 2, 0, Math.PI * 2);
        ctx.fillStyle = rgba(s.color, (1 - tp.age) * 0.25);
        ctx.fill();
      }
      const glow = ctx.createRadialGradient(pos.x, pos.y, 0, pos.x, pos.y, 14);
      glow.addColorStop(0, rgba(s.color, 0.4));
    ...
    for (const ag of heroNodes) {
      const fX = isMobile ? 2 : 5;
      const fY = isMobile ? 1.5 : 4;
      ag.x = ag.baseX + Math.sin(now * 0.5 + ag.phase) * fX;
      ag.y = ag.baseY + Math.cos(now * 0.4 + ag.phase) * fY;
      const dx = heroMouse.x - ag.x;
      const dy = heroMouse.y - ag.y;
      ...
      ctx.beginPath();
      ctx.arc(ag.x, ag.y, rs, 0, Math.PI * 2);
      ctx.strokeStyle = rgba(ag.type.color, 0.06 + b * 0.1);
      ctx.lineWidth = 1;
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.arc(ag.x, ag.y, gs, 0, Math.PI * 2);
      ...
      ctx.shadowColor = rgba(ag.type.color, 0.15 + b * 0.1);
      ctx.shadowBlur = 16;
      ctx.fill();
      ctx.shadowColor = 'transparent';
      ctx.shadowBlur = 0;
      ctx.fillStyle = rgba(ag.type.color, 0.05 + b * 0.03);
      ctx.fill();
      ctx.strokeStyle = rgba(ag.type.color, 0.3 + b * 0.4);
      ctx.lineWidth = 1.5;
      ctx.stroke();
  ...
  heroResize();
  rafId = requestAnimationFrame(animateHero);
  signalInterval = setInterval(spawnHeroSignal, 1200);
